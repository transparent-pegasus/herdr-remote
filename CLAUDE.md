...
- `BASELINE_VERIFICATION_COMMAND`: `cargo clippy --all-targets -- -D warnings && cargo test && (cd web && aube run check && aube run test)`
- `SUPPLEMENTAL_VERIFICATION_COMMANDS`: `cd web && aube run build` when anything under `web/` changes (the Rust server serves `web/dist`)
...
- `FULL_TEST_SUITE_COMMAND`: `cargo test && (cd web && aube run test)`
